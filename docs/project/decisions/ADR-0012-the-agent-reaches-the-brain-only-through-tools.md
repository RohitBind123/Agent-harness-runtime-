# ADR-0012 — The agent reaches the Brain only through tools

**Status:** ACCEPTED
**Date / context:** 2026-09-05, Brain architecture review §13.1.

## Context

An agent needs organizational context. The cheapest way to give it context is
to import the Brain's repository object and call it. The cheapest way is wrong.

## Decision

**The agent reaches the Brain only through TOOLS.** Not through an import. Not
...
the agent package must not import the Brain's store.

## Alternatives considered

| Alternative | Why rejected |
|---|---|
| Direct import of the Brain repository | Destroys the containment property — see below. |
...
If the agent can import the Brain's store directly, the projection stops being
...
- Brain tools live in the agent's tools package, curried like every other tool.
...
- The Brain is placed as a **peer package**, not inside the agent or the
  knowledge layer.
